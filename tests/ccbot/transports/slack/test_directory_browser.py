...
def _make_session(session_id: str, cwd: str = "/tmp/proj", file_path: str = "/tmp/s.jsonl"):
...
def test_build_session_picker_stores_state():
    from ccbot.transports.slack.handlers.directory_browser import (
        build_session_picker, get_session_picker_state,
    )
...
def test_clear_session_picker_state():
    from ccbot.transports.slack.handlers.directory_browser import (
        build_session_picker, clear_session_picker_state, get_session_picker_state,
    )
    sessions = [_make_session("abc12345")]
...
def test_build_session_picker_stores_pending_text():
    ...
    sessions = [_make_session("abc12345")]
    build_session_picker("U1", sessions, msg_ts="T1", pending_text="hello", thread_ts="TH1")
    state = get_session_picker_state("U1", msg_ts="T1")
