...
def _make_task(content_type="text", parts=None, tool_use_id=None,
               channel="C1", thread_ts="T1", window_id="@0"):
...
def test_cannot_merge_different_window():
    from ccbot.transports.slack.handlers.message_queue import _can_merge_tasks
    assert _can_merge_tasks(_make_task(window_id="@0"), _make_task(window_id="@1")) is False
...
def test_cannot_merge_tool_result():
    from ccbot.transports.slack.handlers.message_queue import _can_merge_tasks
    assert _can_merge_tasks(_make_task(content_type="tool_result"), _make_task()) is False


@pytest.mark.asyncio
async def test_merge_combines_parts():
    from ccbot.transports.slack.handlers.message_queue import MessageTask, _merge_content_tasks
...
async def test_merge_stops_at_limit():
    from ccbot.transports.slack.handlers.message_queue import (
        MERGE_MAX_LENGTH, MessageTask, _merge_content_tasks,
    )
    queue: asyncio.Queue[MessageTask] = asyncio.Queue()
    lock = asyncio.Lock()
